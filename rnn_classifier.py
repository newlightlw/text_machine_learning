...
class rnn_clf(object):
	...
	def __init__(self,config):
		...
		with tf.device('/cpu:0'),tf.name_scope('embedding'):
			embedding = tf.get_variable('embedding',shape=[self.vocab_size,self.hidden_size],
			                            dtype=tf.float32)
	...
	def normal_lstm(self):
		...
		with tf.variable_scope('LSTM'):
			outputs,state = tf.nn.dynamic_rnn(cell,inputs=self.inputs,
	...
	def bi_lstm(self):
		...
		with tf.variable_scope('Bi-LSTM'):
			_,state = tf.nn.bidirectional_dynamic_rnn(cell_fw,cell_bw,
			                                          inputs=self.inputs,

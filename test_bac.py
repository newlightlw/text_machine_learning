...
# File paths
tf.flags.DEFINE_string('test_data_file', './data/Test_data.xlsx', 'Test data file path')
tf.flags.DEFINE_string('run_dir', './runs_bac/1561281249', 'Restore the model from this run')
...
# Load test data
data, labels, lengths, label_dict, vocab_processor = data_helper.load_data(file_path=FLAGS.test_data_file,
...
with graph.as_default():
    ...
    for batch in batches:
        ...
        if params['clf'] == 'cnn':
            ...
            feed_dict = {input_x:x_test,input_y:y_test,batch_size:FLAGS.batch_size,sequence_length:x_length,keep_prob:1.0}
            batch_predictions,batch_accuracy = sess.run([predictions,accuracy],feed_dict)
...
dia_arr = np.eye(16)
dict_file = open("label_dict.jsonlabel_dict.json",'r')
with open(os.path.join(FLAGS.run_dir,'predictions.csv'),'w',encoding='utf-8',newline='') as f:
    ...
    for i in range(len(all_predictions)):
        prediction_val = dia_arr[all_predictions[i]]
        for line in dict_file.readlines():
            if line.strip().split(":")[1] == prediction_val:
                print("prediction labels: %s" % line.strip().split(":")[0])
        print(labels[i])
